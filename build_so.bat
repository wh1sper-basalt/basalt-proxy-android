...
echo === Building Basalt Proxy Rust library for Android (cdylib) ===
...
for %%F in ("%ROOT_DIR%app\src\main\jniLibs\arm64-v8a\libbasaltproxy.so") do (
    echo arm64-v8a:   OK [%%~zF bytes]
)
for %%F in ("%ROOT_DIR%app\src\main\jniLibs\armeabi-v7a\libbasaltproxy.so") do (
    echo armeabi-v7a: OK [%%~zF bytes]
)

echo.
echo === BUILD SUCCESS ===
echo   arm64-v8a:   app\src\main\jniLibs\arm64-v8a\libbasaltproxy.so
echo   armeabi-v7a: app\src\main\jniLibs\armeabi-v7a\libbasaltproxy.so
echo.
exit /b 0